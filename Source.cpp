...
public:
	...
	bool IsGood() { // u can be a good person even if u dont do well in academy :))
...
int main() {
	...
	cout << S1 << endl;
	//cout << "Input data for Student2" << endl;
	//cin >> S2;
	//--------------------------------------------------------------------------------------------------------
	...
	cout << "\nAverage grade of the list is: " << (double)average_of_list << endl; // should be 4.6 but displays just 4 : /
	//--------------------------------------------------------------------------------------------------------
	double max = 0.0;
	...
	for (unsigned int i = 0; i < list_of_students.size(); ) {
		if (!(*itr3).IsGood()) {
			counter_failed++;
			failed.push_back(*itr3);
			list_of_students.erase(itr);
		}
		else {
			i++;
		}
